 ...
 * or view credits. Transitions GameState from default to gameStarted.
 *
 * Typically the first screen users see after splash.
 *
 * Interacts with GameLoop to trigger session start.
 *
...
class StartOverlay(
    ...
    override fun handleInput(event: InputEvent) {
        ...
        if (!isVisible) return
        // TODO: Add logic to check if the user tapped a button
